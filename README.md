...
<video src="github-video/home-test.webm" controls width="100%"></video>

> **Note:** The video above (`github-video/home-test.webm`) was generated by running `npx playwright test` with video recording enabled. Any new block or modification to an existing one can follow the same pattern — add a `tests/*.spec.js` file that covers the relevant scenarios and attach the resulting video to the PR for peer review.
